...
class FoundationManifestTest(unittest.TestCase):
    def raw_api(self):
        return {
            "schemaVersion": 1,
            "namespaces": [
                {"name": "std.foundation", "source": "std/foundation.hal", "definitions": [], "examples": []},
                {"name": "std.foundation.bytes", "source": "std/foundation/bytes.hal", "definitions": [], "examples": []},
                {"name": "std.lib.collection", "source": "std/lib/collection.hal", "definitions": [], "examples": []},
            ],
        }
    ...
    def build(self, migrations=()):
        return module.build_manifest(
            ...
            1,
            "migrations.json",
            [{"alias": "bytes", "target": "std.foundation.bytes", "kind": "namespace-alias", "automatic": True}],
            [{"name": "File", "namespace": "std.native.File", "automaticAlias": "File", "kind": "static-object"}],
            repository="https://github.com/hara-lang/hara",
            source_ref="main",
            commit="a" * 40,
            profiles=["jvm", "rust", "wasm"],
            inventory_path="standard-library.namespaces",
        )
    ...
    def test_inventory_is_authoritative(self):
        ...
        self.assertRegex(manifest["surfaceDigest"], r"^sha256:[0-9a-f]{64}$")
    ...
    def test_registered_namespace_missing_from_source_is_rejected(self):
        with self.assertRaisesRegex(ValueError, "Registered/source API mismatch"):
            module.build_manifest(
                self.raw_api(),
                ["std.foundation", "std.foundation.pretty", "std.lib.collection"],
                [], 0, None, [], [],
                repository="repo", source_ref="main", commit="a" * 40,
                profiles=["rust"], inventory_path="inventory",
            )
    ...
    def test_migration_cannot_still_be_current(self):
        migration = {
            ...
            "status": "moved",
            "replacement": {"kind": "native-static-object", "name": "Bytes"},
...
''')
            ...
            self.assertEqual([item["name"] for item in native], ["File", "Json"])

    def test_digest_is_deterministic(self):
        self.assertEqual(module.digest({"b": 2, "a": 1}), module.digest({"a": 1, "b": 2}))
