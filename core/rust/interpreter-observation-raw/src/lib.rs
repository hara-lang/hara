...
use core::{EvalFiber, EvalFiberState, Value};
...
impl InterpreterContext {
    fn fresh() -> (Self, HashMap<String, Value>) {
        ...
        for name in core::structural_callable_names() {
            foundation.intern(name, core::structural_function_value(name));
        }
        ...
        core::select_namespace_environment(&namespaces, &mut environment, "user");
        for (native_type, methods) in core::NATIVE_TYPES {
            let namespace_name = format!("std.native.{native_type}");
            for method in *methods {
                let dispatch_name = match *native_type {
                ...
                environment.insert(format!("{native_type}/{method}"), function.clone());
                environment.insert(format!("{namespace_name}/{method}"), function);
            }
        }
        (
            Self {
...
impl InterpreterObservationSession {
    ...
    fn snapshot(&self) -> Result<Value, String> {
        ...
        Ok(object([
            ...
            ("retained", Value::Number(safe_i64(self.history.len() as u64))),
    ...
    fn set_history_limit(&mut self, limit: usize) -> Value {
        ...
        object([
            ...
            (
                "dropped",
                Value::Number(safe_i64(self.dropped_history)),
            ),
        ])
    }
...
impl ObservationRuntime {
    ...
    fn dispatch(&mut self, request: &Value) -> Result<Value, String> {
        ...
        match operation.as_str() {
            ...
            "resolve-suspension" => {
                let handle = required_handle(request)?;
                let value = field(request, "value").ok_or_else(|| {
                    "interpreter observation resolve requires value".to_string()
                })?;
                self.session(handle)?.resolve_suspension(value).map(Value::Bool)
            }
            "reject-suspension" => {
                let handle = required_handle(request)?;
                let error = field(request, "error").ok_or_else(|| {
                    "interpreter observation reject requires error".to_string()
                })?;
                self.session(handle)?.reject_suspension(error).map(Value::Bool)
            }
            ...
            "set-observation-limits" => {
                ...
                let display_chars =
                    bounded_usize(request, "displayChars", MAX_DISPLAY_CHARS)?;
                Ok(self
                    .session_mut(handle)?
            ...
            "set-retention-limits" => {
                let handle = required_handle(request)?;
                let history =
                    bounded_usize(request, "history", MAX_RETAINED_BOUNDARIES)?;
...
pub extern "C" fn interpreter_observation_invoke(pointer: *const u8, size: usize) -> u64 {
    let response = if pointer.is_null() {
        encode_response(Err("interpreter observation request pointer is null".into()))
    } else {
        let bytes = unsafe { std::slice::from_raw_parts(pointer, size) };
        match std::str::from_utf8(bytes) {
            Ok(source) => invoke_json(source),
            Err(_) => encode_response(Err(
                "interpreter observation request must be valid UTF-8".into(),
            )),
        }
    };
...
fn session_info(handle: u64, session: &InterpreterObservationSession) -> Value {
    object([
        ...
        (
            "generation",
            Value::Number(safe_i64(session.generation())),
        ),
...
mod tests {
    ...
    fn live_session_runs_real_evaluation_and_retains_bounded_history() {
        ...
        assert_eq!(
            field(&run, "schema"),
            Some(Value::String("hal.interpreter-observation-run/0-alpha".into()))
        );
        assert_eq!(
            invoke(&format!(
        ...
        let history = invoke(&format!(
            "{{\"op\":\"history\",\"handle\":{handle}}}"
        ));
        assert_eq!(
        ...
        assert!(matches!(field(&history, "retained"), Some(Value::Number(value)) if value > 0 && value <= 8));
        assert!(matches!(field(&history, "dropped"), Some(Value::Number(value)) if value > 0));
    ...
    fn pending_promise_can_be_settled_and_resumed() {
        let info = invoke(
            r#"{"op":"start","sessionId":"fixture/await","sourceId":"await.hal","source":"(Coroutine/await (promise/new))"}"#,
        );
        let handle = handle(&info);
        let run = invoke(&format!(
            "{{\"op\":\"run\",\"handle\":{handle},\"boundaryLimit\":1000}}"
    ...
    fn reset_rebuilds_isolated_runtime_and_deep_loop_history_stays_bounded() {
        ...
        let reset = invoke(&format!(
            "{{\"op\":\"reset\",\"handle\":{handle}}}"
        ));
