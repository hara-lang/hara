...
fn definition(form: &Form, line: usize) -> Option<Definition> {
    ...
    if !matches!(kind, "def" | "defn" | "defmacro" | "defprotocol" | "deftype" | "defstruct") {
        return None;
    }
    ...
    let doc = match items.get(cursor) {
        Some(Form::String(value)) => { cursor += 1; value.clone() }
        _ => String::new(),
    };
    while matches!(items.get(cursor), Some(Form::Map(_))) { cursor += 1; }
    ...
    Some(Definition { name, kind: kind.to_owned(), doc, signature, line })
}
    ...
    fn visit(path: &Path, files: &mut Vec<PathBuf>) {
        if path.is_dir() {
            if let Ok(entries) = fs::read_dir(path) {
                for entry in entries.flatten() { visit(&entry.path(), files); }
            }
        } else if path.extension().and_then(|v| v.to_str()) == Some("hal") { files.push(path.to_owned()); }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 3 { return Err("usage: hara-api-doc SOURCE_ROOT TEST_ROOT".into()); }
    ...
    for path in foundation_sources(source_root) {
        ...
        let definitions = read_forms(&source)?.into_iter()
        ...
        let examples = if test_path.exists() {
            read_forms(&fs::read_to_string(test_path)?)?.into_iter().filter_map(|spanned| {
                let Form::List(items) = &spanned.form else { return None };
                (items.first().and_then(symbol) == Some("fact")).then(|| spanned.form.to_string())
            }).collect()
        } else { Vec::new() };
        namespaces.push(Namespace { name, source: relative, definitions, examples });
    }
    print!("{{\"schemaVersion\":1,\"namespaces\":[");
    for (ni, ns) in namespaces.iter().enumerate() {
        if ni > 0 { print!(","); }
        print!("{{\"name\":{},\"source\":{},\"definitions\":[", json(&ns.name), json(&ns.source));
        for (di, def) in ns.definitions.iter().enumerate() {
            if di > 0 { print!(","); }
            print!("{{\"name\":{},\"kind\":{},\"doc\":{},\"signature\":{},\"line\":{}}}",
                json(&def.name), json(&def.kind), json(&def.doc), json(&def.signature), def.line);
        }
        print!("],\"examples\":[");
        for (ei, example) in ns.examples.iter().enumerate() {
            if ei > 0 { print!(","); } print!("{}", json(example));
        }
        print!("]}}");
    }
    println!("]}}");
