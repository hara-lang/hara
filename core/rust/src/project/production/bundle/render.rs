...
pub(super) fn retained_modules(
    build: &ProductionBuild,
) -> Result<Vec<RenderedModule>, String> {
    ...
    for module in analyzed_modules {
        ...
        let namespace_form = prune_namespace_form(
...
fn dependency_target(spec: &Form) -> Option<&str> {
    match spec {
        ...
        Form::List(items)
            if matches!(items.first(), Some(Form::Symbol(head)) if head == "quote") =>
        {
            match items.get(1) {
                Some(Form::Symbol(target)) => Some(target),
                _ => None,
