...
use form::{boolean, keyword, map, nil, number, source_form, string, string_vector, strings, symbol, symbol_vector, symbols, vector};
...
fn unit_form(unit: &UnitAnalysis) -> Form {
    map(vec![
        ...
        (
            "unit/compile-time-edges",
            symbols(&unit.compile_time_edges),
        ),
        ("unit/namespace-edges", symbols(&unit.namespace_edges)),
        (
            "unit/native-primitives",
            strings(&unit.native_primitives),
        ),
...
fn reason_form(reason: &RetentionReason) -> Form {
    map(vec![
        ...
        ("reason/code", keyword(&format!("production/{}", reason.code))),
