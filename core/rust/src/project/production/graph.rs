use super::model::{
    Analysis, BuildPlan, Diagnostic, ModuleAnalysis, RetentionReason, UnitAnalysis, UnitKind,
};
...
pub fn finish_analysis(
    ...
    for unit in &units {
        ...
        } else if unit.kind == UnitKind::Initializer
            && unit.effect != super::model::Effect::Pure
        {
            retain(
                &unit.id,
                None,
        ...
        if !providers.contains_key(entrypoint) {
            diagnostics.push(Diagnostic {
                ...
                module: entrypoint
                    ...
                    .unwrap_or("unknown")
                    .into(),
                location: super::model::SourceLocation {
