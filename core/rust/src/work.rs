...
impl WorkHost {
        ...
        F: FnOnce(WorkContext) -> Result<Value, String> + 'static,
    {
        self.submit_scoped_rejection(options, move |context| {
            task(context).map_err(work_failure)
        })
    }
    ...
    fn submit_with_parent(
        ...
        if parent.as_ref().is_some_and(|parent| !parent.accepts_children()) {
...
impl WorkCancellationToken {
    ...
    pub fn reason(&self) -> Option<Value> {
        self.run
            .upgrade()
            .and_then(|run| run.cancellation.borrow().as_ref().map(|request| request.reason.clone()))
    }

    pub fn check(&self) -> Result<(), PromiseRejection> {
        let Some(run) = self.run.upgrade() else {
            return Err(cancellation_rejection(Value::Keyword("scope-closed".into())));
        };
...
fn resolve_deadline(options: &WorkOptions, parent: Option<&WorkRun>) -> Option<Instant> {
    ...
    let relative = options.timeout.and_then(|timeout| Instant::now().checked_add(timeout));
...
fn deadline_remaining_millis(deadline: Instant) -> u64 {
    u64::try_from(deadline.saturating_duration_since(Instant::now()).as_millis())
        .unwrap_or(u64::MAX)
}
...
mod tests {
    ...
    fn cancellation_cancels_active_promise_and_runs_finalizers_once() {
        ...
        let run = host
            .submit_scoped(WorkOptions::with_id("scope-cancel").unwrap(), move |context| {
    ...
    fn parent_completion_is_released_after_child_completion() {
        ...
        let task_child_source = child_source.clone();
        let parent = host
            .submit_scoped(WorkOptions::with_id("wait-parent").unwrap(), move |context| {
    ...
    fn detached_children_outlive_parent_cancellation() {
        let host = WorkHost::new();
        let parent = host
            .submit_scoped(WorkOptions::with_id("detach-parent").unwrap(), move |context| {
                context.submit_child(
                    WorkOptions {
