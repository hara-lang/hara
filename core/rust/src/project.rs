...
const LOCK_FORMAT: &str = "0.0.0-alpha";
...
pub fn runtime_declaration_digest(project: &Project) -> String {
    ...
    digest_dependencies(&mut declaration, "hara-dependencies", &project.dependencies);
    digest_dependencies(
        &mut declaration,
        "maven-dependencies",
    ...
    for (namespace, extension) in &project.extensions {
        digest_scalar(&mut declaration, "extension-namespace", namespace);
        digest_scalar(
            &mut declaration,
...
fn digest_scalar(output: &mut String, label: &str, value: &str) {
    write!(output, "{label}={}:{}\n", value.len(), value).expect("writing to a String cannot fail");
}
...
fn digest_dependencies(output: &mut String, label: &str, values: &BTreeMap<String, String>) {
...
fn reconcile_runtime_section(project: &Project, form: &mut Form) -> Result<(), String> {
        ...
        .find(|(key, _)| key_name(key).as_deref() == Some(project.active_runtime.as_str()))
    {
        *value = section;
        ...
        sections.push((Form::Keyword(project.active_runtime.clone()), section));
    }
    ensure_map_entry(
        entries,
        "packages",
...
fn validate_runtime_lock(project: &Project, path: &Path) -> Result<(), String> {
    ...
    let section = sections
        ...
        .find(|(key, _)| key_name(key).as_deref() == Some(project.active_runtime.as_str()))
        .map(|(_, value)| value)
        .ok_or_else(|| format!("active :{} lock section is absent", project.active_runtime))?;
    let entries = map(
        section,
        &format!(
