...
| `keep-explicit` | `core/lib/src/std/sandbox.hal` | native `Sandbox` capability | These are explicit native capability adapters. Replacing them with namespace interning would erase the intended boundary and there is no ordinary source namespace to publish. |
